...
class ARIMA_implementation():
    ...
    def __init__(self, stock_choice):
        ...
        #Get the data
        try:
            self.read_data = pd.read_csv('E:\ML\project\Data\individual_stocks_5yr\%s_data.csv' %(str(stock_choice)) )
            ...
            self.read_data = pd.read_csv('E:\ML\project\Data/all_stocks_5yr.csv')
        ...
        self.testing_data = self.stationary_data[int(int(len(self.stationary_data))*.7):int(int(len(self.stationary_data))*.9)]
    ...
    def is_data_fitted(self):
        ...
        try:
            with open('E:\ML\project\Data/best_parameters.json') as paramters:
                #Get the dictionary
