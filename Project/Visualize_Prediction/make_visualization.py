...
class visualization():
    ...
    def get_stock_values(self):
        ...
        with open('E:\ML\project\Data/best_parameters.json') as file_save:
            try:
    ...
    def predict_future_values(self, optimal_values, stationary_data, data):
        ...
        self.number_of_predictions = len(data)-int(int(len(data))*.9)

        updated_data = [x for x in data]
        updated_data=updated_data[:int(int(len(data))*.9)]
        predictions = []

        for i in range(self.number_of_predictions):
            model = sm.tsa.statespace.SARIMAX(updated_data, order = optimal_values, enforce_invertibility=False)
            model_fit = model.fit(disp=0)
            forecast = model_fit.forecast(1)[0]
            updated_data.append(data[int(int(len(data))*.9)+i])
    ...
    def show_graph_forecast(self, predicted_values, data_with_predictions):
        ...
        full_data=full_data[:int(len(full_data)*.9)]
        ...
        plt.show()
        error = mean_squared_error(self.data,full_data)
